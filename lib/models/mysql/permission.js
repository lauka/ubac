	...
	*/
	...
	var sql_get = 'select * from rbac_permission';
		sql_get_by_role = 'select * from rbac_permission where role_id=?;',
		sql_get_by_user = 'select * from rbac_permission where role_id in (select role_id from rbac_role_user where user_id=?);',
		sql_add = 'INSERT INTO rbac_permission SET ?',
		sql_set = 'UPDATE rbac_permission SET ? WHERE ?',
		sql_del = 'DELETE FROM rbac_permission WHERE id=?';
		sql_del_permission_role = 'DELETE FROM rbac_permission_role WHERE permission_id=?';
